...
import {
  getFavoriteCities,
  getComposedFavoriteOffersOffers,
} from "../../selectors/data.js";
...
export const Favorites = ({
  ...
  useEffect(() => {
    checkAuthorization();
    loadFavoriteOffers();
    isAuthRequired && history.push(`/login`);
    return () => {
      isAuthRequired ? history.push(`/login`) : loadFavoriteOffers();
      return null;
    };
  }, []);
...
export default connect(
  ...
  (dispatch) => ({
    addToFavorite: (id, status) =>
      dispatch(ActionCreator.addToFavorite(id, status)),
